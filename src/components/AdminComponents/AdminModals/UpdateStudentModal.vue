...
const closeModal = () => {
  useModalStore().closeUpdateStudentModal()
}
onMounted(() => {
  closeModal()
})
...
const getStudentById = async () => {
  ...
  student_type.value = data[0].student_type;
  gender.value = data[0].gender;
  course.value = data[0].course;
};

const updateStudent = async (e) => {
  e.preventDefault();

  if (!firstname.value || !lastname.value || !region.value || !city.value || !phone.value || !birthday.value || !student_type.value || !gender.value || !course.value) {
...
<template>
  <div v-if="useModalStore().isUpdateStudentModalOpen"
       class="fixed -left-10 -top-10 inset-0 z-40 bg-black opacity-50 min-h-screen dark:bg-slate-700">
  </div>
  <div v-if="useModalStore().isUpdateStudentModalOpen"
       class="fixed -left-4 flex z-50 justify-center items-center w-full">
    <div class="relative p-3 w-full max-w-6xl">
      <div class="px-4 py-2 bg-white rounded-lg shadow dark:bg-slate-800 sm:p-5">
        <div class="flex justify-between items-center pb-1 mb-1 rounded-t border-b sm:mb-2 dark:border-slate-600">
          <h3 class="text-lg font-semibold text-slate-900 dark:text-white">O'quvchini o'zgartirish</h3>
        ...
        <form @submit="updateStudent">
          <div class="grid gap-4 px-2 mb-4 sm:grid-cols-3 overflow-y-auto h-[500px] md:h-full">
            ...
            <div>
              ...
              <select id="student-type" v-model="student_type"
                ...
                <option selected value="Abiturient">Abiturient</option>
              ...
              <ul class="items-center w-full text-sm font-medium text-slate-900 bg-white border border-slate-200 rounded-lg sm:flex dark:bg-slate-700 dark:border-slate-600 dark:text-white">
                <li class="w-full border-b border-slate-200 sm:border-b-0 sm:border-r dark:border-slate-600">
                  <div class="flex items-center pl-3">
                    <input id="male" type="radio" value="Erkak" name="male" v-model="gender"
                ...
                <li class="w-full dark:border-slate-600">
                  <div class="flex items-center pl-3">
                    <input id="female" type="radio" value="Ayol" name="female" v-model="gender"
                           class="w-4 h-4 text-blue-600 bg-slate-100 border-slate-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-slate-700 dark:focus:ring-offset-slate-700 focus:ring-2 dark:bg-slate-600 dark:border-slate-500">
          ...
          <div class="flex items-center sm:flex sm:space-y-0 sm:space-x-4">
            <button type="submit"
                    class="w-auto mx-2 sm:w-auto justify-center text-white inline-flex bg-green-600 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-10 py-2.5 text-center">
              Saqlash
            </button>
            <button type="button" @click="closeModal"
                    class="w-auto sm:w-auto justify-center text-white inline-flex bg-slate-500 hover:bg-slate-700 focus:ring-4 focus:outline-none focus:ring-slate-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
